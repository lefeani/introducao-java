...
-- auto_increment = numeração será gerada pelo próprio 80 (1, 2, 3, 4, ...)
...
create table cliente (
    ...
    telefone varchar (20),
    constraint pk_clinete primary key (codigo)
);
...
-- select [quais colunas] from [qual tabela] where [qual condição]alter
...
create table veiculo(
    ...
    constraint fk_veiculo_clientecliente foreign key (proprietario) references cliente (codigo)
    );

-- inserir dados de veículos
insert into veiculo values (null, 'VW', 'Gol', 1);
insert into veiculo values (null, 'Chevrolet', 'Astra', 1);
insert into veiculo values (null, 'BWM', 'X1', 2);
